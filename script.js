...
window.initTableBooking = () => {
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            const newReservationOrder = {
                id: orderId,
                date: timestamp,
                items: [{
                    name: `Table Allotment (${selectedTable}) at ${resOutlet}`,
                    price: 0,
                    quantity: 1,
                    details: `Reserved for ${name} on ${dateVal} at ${timeVal} (${guestsVal})`
                }],
                total: 'Deposit Paid',
                status: 'Reserved Table'
            };

            orders.unshift(newReservationOrder);
            localStorage.setItem('aromatica_orders', JSON.stringify(orders));

            // Populate & show Success Popup Modal
...
// --- Tricity Restaurant Data ---
    ...
    window.openDetails = (name) => {
        ...
        content.innerHTML = `
            ...
            <div class="grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; align-items: start;">
                <div>
                ...
                <div>
                    <h2 style="font-size: 2.5rem; margin-bottom: 1rem; color: var(--primary-color);">${r.name}</h2>
                    ...
                    <ul style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.8rem; margin-bottom: 2rem; color: var(--text-muted);">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const menuData = [
        { id: 1, category: 'Coffee', name: 'Artisan Latte', price: 8.50, veg: true, image: './images/menu/artisan-latte.png', desc: 'Hand-poured luxury espresso with velvet foam.' },
        { id: 2, category: 'Espresso', name: 'Double Shot Reserve', price: 6.00, veg: true, image: './images/menu/double-shot.png', desc: 'Intense flavor from rare Arabica beans.' },
        { id: 3, category: 'Cappuccino', name: 'Classic Velvet', price: 7.50, veg: true, image: './images/menu/classic-velvet.png', desc: 'Perfectly balanced espresso and steamed milk.' },
        { id: 4, category: 'Cold Coffee', name: 'Iced Hazelnut Brew', price: 9.00, veg: true, image: './images/menu/iced-hazelnut.png', desc: 'Cold-pressed coffee with roasted hazelnut syrup.' },
        { id: 5, category: 'Mocktails', name: 'Sunrise Sparkle', price: 12.00, veg: true, image: './images/menu/sunrise-sparkle.png', desc: 'Citrus blend with organic honey and soda.' },
        { id: 6, category: 'Desserts', name: 'Gold Leaf Cheesecake', price: 18.00, veg: true, image: './images/menu/gold-cheesecake.png', desc: 'Rich New York style with 24k edible gold.' },
        { id: 7, category: 'Pizza', name: 'Truffle Mushroom Pizza', price: 24.00, veg: true, image: './images/menu/truffle-pizza.png', desc: 'Hand-stretched dough with wild mushrooms.' },
        { id: 8, category: 'Pasta', name: 'Creamy Pesto Penne', price: 21.00, veg: true, image: './images/menu/pesto-penne.png', desc: 'Fresh basil pesto with pine nuts and parmesan.' },
        { id: 9, category: 'Indian Cuisine', name: 'Butter Chicken Deluxe', price: 28.00, veg: false, image: './images/menu/butter-chicken.png', desc: 'Classic makhani gravy with tender tandoori chicken.' },
        { id: 10, category: 'Chinese', name: 'Truffle Dim Sums', price: 16.00, veg: true, image: './images/menu/truffle-dimsums.png', desc: 'Hand-folded dumplings with black truffle.' },
        { id: 11, category: 'Starters', name: 'Crispy Lotus Stem', price: 14.00, veg: true, image: './images/menu/lotus-stem.png', desc: 'Honey glazed lotus stems with sesame seeds.' },
        { id: 12, category: 'Premium Specials', name: 'Lobster Thermidor', price: 45.00, veg: false, image: './images/menu/lobster-thermidor.png', desc: 'Luxury lobster tail with cognac cream sauce.' }
    ];
        ...
        grid.innerHTML = filtered.map(item => `
            <div class="menu-item-card">
                ...
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                    <h3>${item.name}</h3>
                    <span style="color: var(--primary-color); font-weight: 700;">₹${item.price.toFixed(2)}</span>
                </div>
    ...
    if (ordersList) {
        ...
        if (orders.length === 0) {
            ordersList.innerHTML = `
                <div class="empty-orders reveal active">
                    <i class="fas fa-receipt"></i>
                    <h2>No Orders Found</h2>
                    <p style="color: var(--text-muted); margin-bottom: 2rem;">You haven't placed any orders yet. Start your luxury journey today.</p>
                    <a href="restaurant.html" class="btn btn-primary">Explore Menu</a>
                </div>
            `;
        } else {
            ordersList.innerHTML = orders.map(order => {
                const isReservation = order.status === 'Reserved Table';
                const badgeClass = isReservation ? 'status-reserved' : 'status-preparing';
                const badgeStyle = isReservation ? 'background: rgba(46, 204, 113, 0.2); color: #2ecc71; border: 1px solid #2ecc71;' : '';
                
                // Advanced premium tracker for food delivery orders
                const trackerHtml = isReservation ? '' : `
                    <div class="tracker-status-bar">
                        ...
                        <div class="tracker-step active">
                            <div class="tracker-icon"><i class="fas fa-fire"></i></div>
                            <span>Preparing</span>
                        </div>
                    ...
                    <div class="glass order-card active-tracker-card reveal active">
                        <div class="order-header" style="border-bottom: none; margin-bottom: 0; padding-bottom: 0;">
                            <div style="display: flex; align-items: center; gap: 1rem;">
                                ...
                                <div>
                                    <span class="order-id" style="font-size: 1.1rem;">${isReservation ? '<i class="fas fa-calendar-check" style="margin-right:8px;"></i> Booking' : 'Order'} #${order.id}</span>
                                    <p style="color: var(--text-muted); font-size: 0.85rem; margin-top: 0.2rem;">Aromatica Luxury Dining &bull; ${order.date}</p>
                                </div>
                            </div>
                            <span class="order-status ${badgeClass}" style="${badgeStyle}">${order.status}</span>
                        </div>
                            ...
                            ${order.items.map(item => `
                                <div class="order-item-row" style="flex-direction: column; gap: 0.3rem;">
                                    <div style="display: flex; justify-content: space-between; width: 100%; color: var(--text-light); font-weight: 600;">
                                        ...
                                        <span>${item.price === 0 ? 'Pre-booked' : '₹' + (item.price * item.quantity).toFixed(2)}</span>
                                    </div>
                                    ${item.details ? `<span style="font-size: 0.85rem; color: var(--primary-color);">${item.details}</span>` : ''}
                                </div>
                            `).join('')}
                        </div>
                        <div class="order-footer">
                            <span style="color: var(--text-muted); font-weight: 600;">${isReservation ? 'Allotment Status' : 'Total Amount'}</span>
                            <span class="order-total" style="${isReservation ? 'color: #2ecc71; font-size:1.2rem;' : 'color: var(--primary-color); font-size: 1.2rem;'}">${order.total}</span>
                        </div>
                        <div style="margin-top: 1.5rem; display: flex; gap: 1rem; flex-wrap: wrap;">
                            <button class="btn btn-outline" style="flex: 1; padding: 0.8rem; font-size: 0.9rem;" onclick="window.print()"><i class="fas fa-file-invoice"></i> ${isReservation ? 'Print Pass' : 'View Receipt'}</button>
                            <button class="btn btn-primary" style="flex: 1; padding: 0.8rem; font-size: 0.9rem;" onclick="alert('${isReservation ? 'Your table is fully secured and prepared for your arrival!' : 'Live GPS tracking initiated...'}')"><i class="fas fa-map-marker-alt"></i> ${isReservation ? 'View Floor Map' : 'Track Live'}</button>
                        </div>
                    </div>
                `;
            }).join('');
    ...
    // --- Custom Cursor & Cinematic Parallax Ecosystem ---
